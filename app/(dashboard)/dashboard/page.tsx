...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500 pb-12">
      ...
      {!gardenId ? (
        <div className="flex h-[400px] items-center justify-center text-muted-foreground border rounded-xl border-dashed font-medium">
          {t("dashboard.selectGardenPrompt")}
        </div>
      ) : isLoading ? (
        ...
        <div className="flex h-[400px] flex-col items-center justify-center text-red-500 border rounded-xl border-dashed border-red-200 bg-red-50 gap-2">
          ...
          <p className="font-medium">{t("dashboard.failedAnalytics")}</p>
          <p className="text-sm opacity-80">{t("dashboard.checkConnection")}</p>
        </div>
      ) : stats ? (
        <>
