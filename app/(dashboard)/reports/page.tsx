...
import { GardenStats, HarvestStats, InventoryItem, PaginatedResponse } from "@/types";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Comprehensive analytics on crop health, harvest yields, and inventory status
          </p>
        </div>
      ...
      {!gardenId ? (
        <div className="flex h-[300px] flex-col items-center justify-center text-muted-foreground border rounded-xl border-dashed gap-2">
          ...
          <p className="font-medium">Select a garden to view reports</p>
          <p className="text-xs text-muted-foreground">Choose a garden from the dropdown above to load data</p>
        </div>
        ...
        <div className="flex h-[300px] flex-col items-center justify-center text-muted-foreground border rounded-xl border-dashed gap-4">
          ...
          <p className="text-sm">Compiling analytical reports...</p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-xl border bg-card text-card-foreground p-5 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Crop Health Ratio</p>
                  <h3 className="text-3xl font-bold mt-2 text-green-600">{healthyPercentage}%</h3>
                  <p className="text-xs text-muted-foreground mt-1">
                    {stats?.by_status.HEALTHY || 0} of {stats?.total_plants || 0} plants healthy
                  </p>
                </div>
            ...
            <div className="rounded-xl border bg-card text-card-foreground p-5 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Total Yield</p>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {harvestStats?.total_records || 0} harvest batches recorded
                  </p>
                </div>
            ...
            <div className="rounded-xl border bg-card text-card-foreground p-5 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Stock Warnings</p>
            ...
            <div className="rounded-xl border bg-card text-card-foreground p-5 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Early Disease Alerts</p>
                  <h3 className={`text-3xl font-bold mt-2 ${stats?.alerts && stats.alerts.length > 0 ? "text-red-600" : "text-gray-900"}`}>
                    {stats?.alerts?.length || 0}
                  </h3>
                  <p className="text-xs text-muted-foreground mt-1">
                    Active outbreak pattern warnings
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-semibold text-lg flex items-center gap-2">
                    <TrendingUp className="h-5 w-5 text-blue-600" /> Weekly Care & Health Trend
                  </h3>
                  <p className="text-xs text-muted-foreground">Historical tracking of care logs & sick tree counts</p>
                </div>
              </div>
                ...
                <div className="h-[260px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={stats.weekly_trend}>
                      ...
                      <RechartsTooltip
                        labelFormatter={(val) => `Week starting ${format(new Date(val), "MMM d, yyyy")}`}
                      />
                      <Line type="monotone" dataKey="reports" name="Care Logs" stroke="#2563eb" strokeWidth={2.5} dot={{ r: 4 }} />
                      <Line type="monotone" dataKey="sick" name="Sick Plants" stroke="#dc2626" strokeWidth={2} dot={{ r: 3 }} />
                      <Line type="monotone" dataKey="watching" name="Watching" stroke="#eab308" strokeWidth={2} dot={{ r: 3 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              ) : (
            ...
            <div className="rounded-xl border bg-card p-6 shadow-sm flex flex-col justify-between">
              <div>
                <h3 className="font-semibold text-lg flex items-center gap-2 mb-1">
                  <Activity className="h-5 w-5 text-green-600" /> Crop Status Distribution
                </h3>
                <p className="text-xs text-muted-foreground mb-4">Overall health breakdown of current plants</p>
                ...
                {Object.entries(STATUS_COLORS).map(([status, color]) => {
                  ...
                  return (
                    <div key={status} className="flex items-center gap-2">
                      ...
                      <span className="text-muted-foreground">{status}: <strong className="text-foreground">{count}</strong></span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
            ...
            <div className="rounded-xl border bg-card p-6 shadow-sm">
              <h3 className="font-semibold text-lg flex items-center gap-2 mb-1">
                <Calendar className="h-5 w-5 text-emerald-600" /> Harvest Yield by Season
              </h3>
              <p className="text-xs text-muted-foreground mb-4">Yield distribution across seasonal harvest cycles</p>
                ...
                <div className="h-[220px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={harvestStats.by_season}>
                      ...
                      <YAxis tick={{ fontSize: 12 }} />
                      <RechartsTooltip formatter={(val) => [`${val} kg`, "Yield"]} />
                      <Bar dataKey="quantity_kg" name="Yield (kg)" fill="#059669" radius={[6, 6, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ) : (
                <div className="h-[220px] flex items-center justify-center text-xs text-muted-foreground">
                  No seasonal harvest data recorded yet.
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border bg-card p-6 shadow-sm">
              <h3 className="font-semibold text-lg flex items-center gap-2 mb-1">
                <Award className="h-5 w-5 text-amber-600" /> Quality Grade Analysis
              </h3>
              <p className="text-xs text-muted-foreground mb-4">Yield classification by crop quality grade</p>
                ...
                <div className="h-[220px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={harvestStats.by_quality}>
                      ...
                      <YAxis tick={{ fontSize: 12 }} />
                      <RechartsTooltip formatter={(val) => [`${val} kg`, "Quantity"]} />
                      <Bar dataKey="quantity_kg" name="Yield (kg)" fill="#d97706" radius={[6, 6, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ) : (
                <div className="h-[220px] flex items-center justify-center text-xs text-muted-foreground">
                  No quality grade data recorded yet.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border bg-card p-6 shadow-sm">
            <h3 className="font-semibold text-lg flex items-center gap-2 mb-1">
              <PackageX className="h-5 w-5 text-amber-600" /> Stock Replenishment Report
            </h3>
            <p className="text-xs text-muted-foreground mb-4">Agricultural supplies below required minimum safety stock levels</p>
            ...
            {inventoryWarnings && inventoryWarnings.length > 0 ? (
              ...
              <div className="p-4 rounded-lg bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-300 text-xs font-medium flex items-center gap-2">
                ...
                All inventory supply stock levels are optimal and above minimum threshold.
              </div>
            )}
          </div>
        </div>
